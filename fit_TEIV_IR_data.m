%% Fit TEIV Model to synthetic data from IR model, with varied immune response parameters 
...
Theta_vec = zeros(l1+l2,7);
err_vec = zeros(l1+l2,1);
Theta2 = [2e-06,0.01,2,10,4,7e9,434];
Theta3 = [2e-06,0.005,2,10,4,7e9,434];
Theta0 = Theta2;
...
% if v == 5 && par_vec(i) < 0.8   
% Theta0 = Theta3;
% end

[V_vec, Frac_vec] = gen_data_IR(sigma,t_end,1,Theta_s);
dat = [[1:t_end]',V_vec'];
[theta,Theta1,~,ti,Yf,tf,err] = fit_TEIV(dat,Theta0,6,0,0,0);
Theta0 = Theta1;
...
[V_vec, Frac_vec] = gen_data_IR(sigma,t_end,1,Theta_s);
dat = [[1:t_end]',V_vec'];
[theta,Theta1,~,~,~,~,err] = fit_TEIV(dat,Theta0,6,1,1,0);
ylim([1e1 1e7])
yticks([1e1, 1e4, 1e7])
xticks([0, 6, 12])
...
if sav == 1
filename = ['TEIV_Sim_',char(label1(v))];
...
ylabel('Relative TEIV Parameter Values','FontSize',22)
% legend({'$\beta$','$p$','$c$','$V_0$'},'FontSize',22,'Location','Southeast');
legend(p,{'$\beta$','$p$','$\delta$','$c$'},'FontSize',22,'Location','Southeast');
...
ylim([1e-2 1e2])
if sav == 1
filename = ['TEIV_Par_',char(label1(v))];
saveas(gcf,filename,'epsc')
savefig(gcf,filename)
end
...
if sav == 1
filename = ['TEIV_Err_',char(label1(v))];
saveas(gcf,filename,'epsc')
savefig(gcf,filename)
save(filename)
end
